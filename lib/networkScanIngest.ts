...
export type NetworkScanIngestResult = {
  ok: true;
  vrmSiteId: string;
  discovered: number;
  updated: number;
  ...
  cellularInserted: boolean;
  ignored: string[];
};

export class NetworkScanIngestError extends Error {
  status: number;

  constructor(message: string, status: number) {
    super(message);
    this.name = 'NetworkScanIngestError';
    this.status = status;
  }
}
...
async function sendAlertWebhook(payload: Record<string, unknown>) {
  ...
  try {
    ...
    if (!response.ok) {
      console.error('[network-scan-ingest] alert webhook failed:', response.status, await response.text());
    }
  } catch (error) {
    console.error('[network-scan-ingest] alert webhook error:', error);
  }
}

export async function ingestNetworkScanPayload(body: CerboNetworkScanPayload): Promise<NetworkScanIngestResult> {
  ...
  if (!body?.vrmSiteId || (!hasCellularReport && reports.length === 0)) {
    throw new NetworkScanIngestError('Invalid payload', 400);
  }
  ...
  if (vrmError) {
    console.error('[network-scan-ingest] vrm device lookup error:', vrmError.message);
    throw new NetworkScanIngestError('Lookup failed', 500);
  }
  if (!vrmDevice) {
    throw new NetworkScanIngestError('Unknown vrmSiteId', 404);
  }
  ...
  if (managedError) {
    console.error('[network-scan-ingest] managed device lookup error:', managedError.message);
    if (!hasCellularReport) {
      throw new NetworkScanIngestError('Managed device lookup failed', 500);
    }
  }
  ...
  if (hasCellularReport) {
    ...
    if (hasSignalValue) {
      ...
      if (cellularError) {
        console.error('[network-scan-ingest] cellular report insert error:', cellularError.message);
      } else {
  ...
  const applyManagedStatus = async (
    ...
    if (updateError) {
      console.error('[network-scan-ingest] update error:', updateError.message);
      return false;
    }
  ...
  for (const report of reports) {
    ...
    if (discoveryError) {
      console.error('[network-scan-ingest] discovery upsert error:', discoveryError.message);
    } else {
      discovered += 1;
    }
    ...
    await applyManagedStatus(managedDevice, status, latencyMs, detail, ipAddress);
  }
  ...
  if (isFullScan) {
    ...
    if (previousDiscoveryError) {
      console.error('[network-scan-ingest] previous discovery lookup error:', previousDiscoveryError.message);
    } else {
      for (const device of previouslyDiscovered ?? []) {
        ...
        if (discoveryOfflineError) {
          console.error('[network-scan-ingest] discovery offline update error:', discoveryOfflineError.message);
        } else {
          markedOffline += 1;
        }
      }
  ...
  return {
    ok: true,
    vrmSiteId: String(vrmDeviceRow.vrm_site_id),
    discovered,
    updated,
    markedOffline,
